...
MAIN_HAL_SRC="./Drivers/STM32F1xx_HAL_Driver/Src"
MAIN_HAL_INC="./Drivers/STM32F1xx_HAL_Driver/Inc"
...
check_exist_f ${MAIN_LINKERS} STM32F1*.ld
check_exist_f ${MAIN_CMSIS_CORE} *.h
check_exist_f ${MAIN_CMSIS_DEV_INC} *.h
check_exist_f ${MAIN_CMSIS_DEV_SRC} system_stm32f1xx.c
check_exist_f ${MAIN_CMSIS_DEV_GCC} startup_stm32f1*.s
echo 
...
DEVICE_HOME=$(find $1 -type d -name STM32F1xx) 
...
NEW_HAL_D=$(find $1 -type d -name STM32F1xx_HAL_Driver )
...
check_exist_f ${NEW_CMSIS_DEV_GCC_D} startup_stm32f1*.s
echo
